...
function Invoke-SecureUninstall {
    ...
    "=== BATCRelayBot Removal Log ===" | Out-File $logPath -Force
    "Started: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Add-Content $logPath
    ""  | Add-Content $logPath
    ...
    try {
        ...
        if ($botProcess) {
            ...
            "[OK] Bot process stopped" | Add-Content $logPath
    ...
    if (Test-Path $configPath) {
        try {
            ...
            if (Test-Path $sdeleteExe) {
                ...
                "[OK] config.json securely deleted (3-pass SDelete)" | Add-Content $logPath
                ...
                "[OK] config.json deleted (multi-pass overwrite)" | Add-Content $logPath
    ...
    try {
        if (Test-Path $BotPath) {
            ...
            "[OK] Deleted $count files from $BotPath" | Add-Content $logPath
    ...
    try {
        ...
        if (Test-Path $appDataPath) {
            ...
            "[OK] Cleaned AppData\BATCRelayBot" | Add-Content $logPath
            Write-Host "      DONE" -ForegroundColor Green
        } else {
        ...
        if ((Test-Path $pipCache) -and $DependencyChoices.RemovePython) {
            ...
            "[OK] Cleaned pip cache" | Add-Content $logPath
        }
    } catch {
        Write-Host "      WARNING: $_" -ForegroundColor Yellow
    }
    ...
    if ($DependencyChoices.RemovePython) {
        try {
            ...
            "[OK] Python uninstalled" | Add-Content $logPath
            $depsRemoved++
        } catch {
    ...
    if ($DependencyChoices.RemoveFFmpeg) {
        try {
            ...
            "[OK] FFmpeg uninstalled" | Add-Content $logPath
            $depsRemoved++
        } catch {
    ...
    if ($DependencyChoices.RemoveModule) {
        try {
            ...
            "[OK] PowerShell module uninstalled" | Add-Content $logPath
            $depsRemoved++
        } catch {
    ...
    if (-not $success) {
        "[OK] Pre-deletion verification: Installation directory exists, proceeding with removal" | Add-Content $logPath
    }
